extends SceneTree
## render_3q_hd.gd — render HD do soph_dressed_mesh.glb texturizado, 3/4 (az=315)
## e frente (az=0), fundo TRANSPARENTE (pra compor verde/branco depois no PIL).
##   xvfb-run -a "$GODOT" --rendering-driver opengl3 -s tools/rig3d/render_3q_hd.gd
const GLB := "res://tools/rig3d/in/soph_dressed_mesh.glb"
const OUT := "res://tools/rig3d/out/textured/"
const RES := Vector2i(1024, 1536)
const AZS := [315, 0]
const EL := 7.0
...
func _initialize() -> void:
	...
	env.ambient_light_color = Color(1,1,1); env.ambient_light_energy = 1.25
	var we := WorldEnvironment.new(); we.environment = env; get_root().add_child(we)
	var sun := DirectionalLight3D.new()
	sun.rotation_degrees = Vector3(-40,25,0); sun.light_energy = 0.9
	sun.shadow_enabled = false
	get_root().add_child(sun)
	# fill leve do outro lado p/ nao perder mao/perfil na sombra
	var fill := DirectionalLight3D.new()
	fill.rotation_degrees = Vector3(-20,200,0); fill.light_energy = 0.5
	...
	if gdoc.append_from_file(ProjectSettings.globalize_path(GLB), st) != OK:
		push_error("[hd] falha GLB"); quit(1); return
...
func _process(_d: float) -> bool:
	...
	if _f % 4 == 0:
		get_root().get_texture().get_image().save_png(
			ProjectSettings.globalize_path(OUT + "hd_az_%03d.png" % AZS[_i]))
		print("shot hd az=", AZS[_i])
		_i += 1
	return false
